...
wavelet_list = ['haar', 'db1']
psnr_list = [[0]*len(wavelet_list)]*(len(levels)+1)
print(psnr_list)
ssim_list = [[0]*len(wavelet_list)]*(len(levels)+1)
print(ssim_list)
...
for n in levels:
    ...
    for i in wavelet_list:
        ...
        psnr_list[n][count] = float(psnr_A)
        ssim_list[n][count] = float(ssim_A)
        count += 1
...
for i in range(1, len(psnr_list)):
    plt.plot(wavelet_list, psnr_list[i], label = 'Level ' + str(i))
